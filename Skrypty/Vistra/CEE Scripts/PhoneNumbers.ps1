...
$users = @()
foreach ($ou in $ous){
$users += get-aduser -filter * -SearchBase $ou -Properties telephonenumber, initials | select  telephonenumber, initials, name, userprincipalname 
}
$path = 'C:\CEE scripts\phoneNumbers.csv'
