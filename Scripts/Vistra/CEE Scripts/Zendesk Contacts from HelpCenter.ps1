...
$articles = Get-ZendeskData -headers $headers -url $FullArticleUrl 

$vistraGroup = $articles | ? {$_.section_id -eq 201595111} | select * -Unique
...
ArticlesToHtml -articles $vistraGroup -path $path
