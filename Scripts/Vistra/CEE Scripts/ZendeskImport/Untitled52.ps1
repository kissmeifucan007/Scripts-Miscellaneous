...
"OU=Berlin,OU=Vistra,DC=work,DC=local"
...
foreach ($ou in $UserOUs){
   $CEEAccounts+= get-aduser -Filter {enabled -eq $true} -SearchBase $ou | where {(Remove-StringLatinCharacters $_.name) -cmatch $regex} | select name
}

$CEEAccounts.Count

$CEEAccounts | Out-GridView
