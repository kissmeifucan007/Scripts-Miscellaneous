$file = 'C:\Users\mwojewoda\OneDrive - Sii Sp. z o.o\Documents\SII\Wzór prezentacji\Corporate Presentation Generic EN.pptx'
$parent = split-path -Path $file -Parent 
$directory = Join-Path -Path $parent "unzipped"
Expand-Archive -Path $file -DestinationPath $directory 
$media = Join-Path -path  $directory -childPath "\ppt\media"
Get-ChildItem $media | select * | Out-GridView
...
$metadata = Get-FileMetaData   $media